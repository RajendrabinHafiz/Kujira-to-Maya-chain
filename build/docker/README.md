...
```bash
docker run -e CHAIN_ID=thorchain-mainnet-v1 -e NET=mainnet registry.gitlab.com/mayachain/mayanode:chaosnet-multichain
```
...
docker compose cp thornode:/root/.mayanode/config/genesis.json .
...
thornode tx mayachain mimir NUMBEROFNEWNODESPERCHURN 1 --from dog $TX_FLAGS
```
