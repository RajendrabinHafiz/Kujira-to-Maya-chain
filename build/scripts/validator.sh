...
if [ ! -f ~/.mayanode/config/genesis.json ]; then
  ...
  if [ "$NET" = "mocknet" ]; then
    ...
    # bond our liquidity
    until printf "%s\n" "$SIGNER_PASSWD" | mayanode tx mayachain deposit 100000000 cacao "bond:$NODE_ADDRESS" --node tcp://"$PEER":26657 --from "$SIGNER_NAME" --keyring-backend=file --chain-id "$CHAIN_ID" --yes --gas 'auto' >log && grep -v 'out of gas' log; do
      sleep 5
    done
