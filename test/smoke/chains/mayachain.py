...
class MnemonicKey(TerraMnemonicKey):
    ...
    def acc_address(self) -> AccAddress:
        """Mayachain Bech32 account address.
        ...
        return AccAddress(get_bech("tmaya", self.raw_address.hex()))
    ...
    def acc_pubkey(self) -> AccPubKey:
        ...
        return AccPubKey(get_bech("tmayapub", self.raw_pubkey.hex()))


class MockMayachain(HttpClient):
    """
    A local simple implementation of mayachain chain
    """
    ...
    def __init__(self, base_url):
        ...
        self.lcd_client.chain_id = "mayachain"
        self.init_wallets()
    ...
    def get_balance(self, address, asset=Asset("MAYA.CACAO")):
        """
        Get MAYA balance for an address
        """
        if "VAULT" == get_alias("MAYA", address):
            balance = self.fetch("/mayachain/balance/module/asgard")
            for coin in balance:
    ...
    def transfer(self, txns):
        ...
        for txn in txns:
            ...
            txn.gas = [Coin("MAYA.CACAO", 2000000000)]
            ...
            tx_options = CreateTxOptions(
                ...
                fee=Fee(20000000, "0ucacao"),  # gas limit 0.2ucacao fee 0ucacao,
            )
...
class Mayachain(GenericChain):
    ...
    name = "MAYAChain"
    chain = "MAYA"
    coin = Asset("MAYA.CACAO")

    def __init__(self):
        super().__init__()

        # seeding the users, these seeds are established in build/scripts/genesis.sh
        acct = Account("tmaya1z63f3mzwv3g75az80xwmhrawdqcjpaekkcgpz9")
        acct.add(Coin(self.coin, 500000000000000))
        self.set_account(acct)

        acct = Account("tmaya1wz78qmrkplrdhy37tw0tnvn0tkm5pqd6z6lxzw")
        acct.add(Coin(self.coin, 2500000000000100))
        self.set_account(acct)

        acct = Account("tmaya1xwusttz86hqfuk5z7amcgqsg7vp6g8zhsk2n26")
        acct.add(Coin(self.coin, 509000000000000))
        self.set_account(acct)
