...
class MockKuji(HttpClient):
    ...
    def __init__(self, base_url):
        ...
        self.lcd_client.chain_id = "harpoon-4"
                ...
                CreateTxOptions(
                    msgs=[
                        MsgSend(
                            ...
                            "10000ukujira",  # send 0.01 kujira
                        )
                    ],
                    sequence=sequence,
                    memo="fee generation",
                    fee=Fee(200000, "20000ukujira"),  # fee 0.02 kujira
                )
            )
    ...
    def transfer(self, txn):
        ...
        tx = wallet.create_and_sign_tx(
            CreateTxOptions(
                ...
                memo=txn.memo,
                fee=Fee(200000, "20000ukujira"),
            )
        )

        result = self.lcd_client.tx.broadcast(tx)
        txn.id = result.txhash
        txn.gas = [Coin("KUJI.kujira", self.default_gas)]
