...
func (b *BlockScanner) isChainPaused() bool {
	...
	haltHeight, err := b.mayachainBridge.GetMimir(fmt.Sprintf("Halt%sChain", b.cfg.ChainID))
	if err != nil {
	...
	solvencyHaltHeight, err = b.mayachainBridge.GetMimir(fmt.Sprintf("SolvencyHalt%sChain", b.cfg.ChainID))
	if err != nil {
	...
	globalHaltHeight, err := b.mayachainBridge.GetMimir("HaltChainGlobal")
	if err != nil {
	...
	nodeHaltHeight, err = b.mayachainBridge.GetMimir("NodePauseChainGlobal")
	if err != nil {
		b.logger.Error().Err(err).Msg("fail to get mimir setting NodePauseChainGlobal")
	}
	thorHeight, err = b.mayachainBridge.GetBlockHeight()
	if err != nil {
		b.logger.Error().Err(err).Msg("fail to get BASEChain block height")
	}
...
func (b *BlockScanner) scanBlocks() {
	...
	lastMimirCheck := time.Now().Add(-constants.MayachainBlockTime)
	isChainPaused := false
	...
	for {
		...
		default:
			...
			if time.Since(lastMimirCheck) >= constants.MayachainBlockTime {
...
func (b *BlockScanner) FetchLastHeight() (int64, error) {
	...
	if err := b.mayachainBridge.WaitToCatchUp(); err != nil {
		return 0, err
	}
	if b.mayachainBridge != nil {
		var height int64
		if b.cfg.ChainID.Equals(common.BASEChain) {
			height, _ = b.mayachainBridge.GetBlockHeight()
		} else {
			height, _ = b.mayachainBridge.GetLastObservedInHeight(b.cfg.ChainID)
		}
		if height > 0 {
			return height, nil
		}
