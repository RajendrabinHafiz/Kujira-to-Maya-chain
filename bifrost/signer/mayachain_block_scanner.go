...
type ThorchainBlockScan struct {
	...
	thorchain      mayaclient.MayachainBridge
	errCounter     *prometheus.CounterVec
	pubkeyMgr      pubkeymanager.PubKeyValidator
}

// NewThorchainBlockScan create a new instance of thorchain block scanner
func NewThorchainBlockScan(cfg config.BifrostBlockScannerConfiguration, scanStorage blockscanner.ScannerStorage, thorchain mayaclient.MayachainBridge, m *metrics.Metrics, pubkeyMgr pubkeymanager.PubKeyValidator) (*ThorchainBlockScan, error) {
	if scanStorage == nil {
	...
	return &ThorchainBlockScan{
		logger:         log.With().Str("module", "blockscanner").Str("chain", "MAYA").Logger(),
		wg:             &sync.WaitGroup{},
		...
		thorchain:      thorchain,
		errCounter:     m.GetCounterVec(metrics.MayachainBlockScannerError),
		pubkeyMgr:      pubkeyMgr,
	}, nil
}
