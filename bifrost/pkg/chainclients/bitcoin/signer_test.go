...
func (s *BitcoinSignerSuite) SetUpTest(c *C) {
	...
	s.server = httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		if req.RequestURI == "/mayachain/vaults/tmayapub1addwnpepqwznsrgk2t5vn2cszr6ku6zned6tqxknugzw3vhdcjza284d7djp59sf99q/signers" { // nolint
			_, err := rw.Write([]byte("[]"))
			c.Assert(err, IsNil)
		} else if strings.HasPrefix(req.RequestURI, "/mayachain/vaults") && strings.HasSuffix(req.RequestURI, "/signers") {
			httpTestHandler(c, rw, "../../../../test/fixtures/endpoints/tss/keysign_party.json")
		} else if req.RequestURI == "/mayachain/version" {
...
func (s *BitcoinSignerSuite) TestSignTxWithTSS(c *C) {
	...
	txOutItem := stypes.TxOutItem{
		Chain:       common.BTCChain,
		ToAddress:   addr,
		VaultPubKey: "tmayapub1addwnpepqwznsrgk2t5vn2cszr6ku6zned6tqxknugzw3vhdcjza284d7djp59sf99q",
		Coins: common.Coins{
			common.NewCoin(common.BTCAsset, cosmos.NewUint(10)),
		},
		MaxGas: common.Gas{
	...
	thorKeyManager := &tss.MockMayachainKeyManager{}
