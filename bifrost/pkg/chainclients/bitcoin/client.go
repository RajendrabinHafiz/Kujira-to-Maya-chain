...
const (
	...
	// EstimateAverageTxSize for BASEChain the estimate tx size is hard code to 1000 here , as most of time it will spend 1 input, have 3 output
...
// the transaction had been removed from chain,  chain client should report to mayachain
...
func (c *Client) FetchTxs(height int64) (types.TxIn, error) {
	...
	if c.IsBlockScannerHealthy() {
		if err := c.ReportSolvency(height); err != nil {
			c.logger.Err(err).Msgf("fail to send solvency info to BASEChain")
		}
	}
	txIn.Count = strconv.Itoa(len(txIn.TxArray))
...
func (c *Client) extractTxs(block *btcjson.GetBlockVerboseTxResult) (types.TxIn, error) {
	...
	for idx, tx := range block.Tx {
		...
		if txInItem.Coins[0].Amount.LTE(cosmos.NewUint(c.chain.DustThreshold().Uint64())) {
			c.logger.Info().Msgf("tx: %s is dust, ignore", txInItem.Tx)
			continue
		}
...
func (c *Client) ignoreTx(tx *btcjson.TxRawResult) bool {
	...
	// there are more than two output with value in it, not BASEChain format
