...
const (
	...
	DefaultCoinbaseValue          = 1000
	MaxAsgardAddresses            = 100
	EstimateAverageTxSize         = 304
	RpcErrorBlockHeightOutOfRange = "Block height out of range"
)
...
func (c *Client) getTxIn(tx *btcjson.TxRawResult, height int64) (txInItem types.TxInItem, err error) {
	if shouldIgnore, ignoreReason := c.ignoreTx(tx); shouldIgnore {
...
func (c *Client) ignoreTx(tx *btcjson.TxRawResult) (shouldIngore bool, ignoreReason string) {
	if len(tx.Vin) == 0 {
		return true, "0 vins"
	}
	if len(tx.Vout) == 0 {
...
func (c *Client) getBlockRequiredConfirmation(txIn types.TxIn, height int64) (int64, error) {
	...
	if totalFeeAndSubsidy == 0 {
		cbValue, err := dashutil.NewAmount(DefaultCoinbaseValue)
		if err != nil {
