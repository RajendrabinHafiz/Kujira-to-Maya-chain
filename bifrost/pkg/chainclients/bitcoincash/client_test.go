...
func (s *BitcoinCashSuite) SetUpTest(c *C) {
	...
	s.server = httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		if req.RequestURI == "/" { // nolint
		...
		} else if strings.HasPrefix(req.RequestURI, "/mayachain/node/") {
			httpTestHandler(c, rw, "../../../../test/fixtures/endpoints/nodeaccount/template.json")
		} else if req.RequestURI == "/mayachain/lastblock" {
			...
			c.Assert(err, IsNil)
		} else if strings.HasPrefix(req.RequestURI, mayaclient.AsgardVault) {
			httpTestHandler(c, rw, "../../../../test/fixtures/endpoints/vaults/asgard.json")
		} else if req.RequestURI == "/mayachain/mimir/key/MaxUTXOsToSpend" {
