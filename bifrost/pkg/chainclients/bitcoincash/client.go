...
const (
	...
	// EstimateAverageTxSize for BASEChain the estimate tx size is hard code to 250 here , as most of time it will spend 1 input, have 3 output
...
func (c *Client) extractTxs(block *btcjson.GetBlockVerboseTxResult) (types.TxIn, error) {
	...
	for idx, tx := range block.Tx {
		...
		if txInItem.Coins[0].Amount.LTE(c.chain.DustThreshold()) {
			continue
		}
