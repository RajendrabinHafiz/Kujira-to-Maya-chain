...
func NewBinance(thorKeys *mayaclient.Keys, cfg config.BifrostChainConfiguration, server *tssp.TssServer, mayachainBridge mayaclient.MayachainBridge, m *metrics.Metrics) (*Binance, error) {
	...
	b := &Binance{
		...
		cfg:             cfg,
		cdc:             mayaclient.MakeLegacyCodec(),
	...
	b.bnbScanner, err = NewBinanceBlockScanner(b.cfg.BlockScanner, b.storage, b.isTestNet, b.mayachainBridge, m, b.ReportSolvency)
	if err != nil {
		return nil, fmt.Errorf("fail to create block scanner: %w", err)
	}

	b.blockScanner, err = blockscanner.NewBlockScanner(b.cfg.BlockScanner, b.storage, m, b.mayachainBridge, b.bnbScanner)
	if err != nil {
		return nil, fmt.Errorf("fail to create block scanner: %w", err)
	}
...
func (b *Binance) Start(globalTxsQueue chan stypes.TxIn, globalErrataQueue chan stypes.ErrataBlock, globalSolvencyQueue chan stypes.Solvency) {
	...
	b.wg.Add(1)
	// Binance has 300ms block time , which is way faster than BASEChain block time , thus , discover block height in solvency runner should be more aggressive
	go runners.SolvencyCheckRunner(b.GetChain(), b, b.mayachainBridge, b.stopchan, b.wg, b.GetConfig().BlockScanner.BlockHeightDiscoverBackoff)
}
...
func (b *Binance) SignTx(tx stypes.TxOutItem, thorchainHeight int64) ([]byte, []byte, error) {
	...
	if strings.EqualFold(tx.Memo, mayachain.NewYggdrasilReturn(thorchainHeight).String()) {
...
func (b *Binance) signMsg(signMsg btx.StdSignMsg, from string, poolPubKey common.PubKey, thorchainHeight int64, txOutItem stypes.TxOutItem) ([]byte, error) {
	...
	if errors.As(err, &keysignError) {
		...
		// fail to sign a message , forward keysign failure to BASEChain , so the relevant party can be blamed
		txID, errPostKeysignFail := b.mayachainBridge.PostKeysignFailure(keysignError.Blame, thorchainHeight, txOutItem.Memo, txOutItem.Coins, poolPubKey)
...
func (b *Binance) ReportSolvency(bnbBlockHeight int64) error {
	...
	asgardVaults, err := b.mayachainBridge.GetAsgards()
	if err != nil {
	...
	for _, asgard := range asgardVaults {
		...
		case <-time.After(constants.MayachainBlockTime):
			b.logger.Info().Msgf("fail to send solvency info to BASEChain, timeout")
		}
	}
