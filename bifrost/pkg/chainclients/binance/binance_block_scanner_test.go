...
import (
	...
	btypes "gitlab.com/mayachain/mayanode/bifrost/pkg/chainclients/binance/types"
...
func getMultiSendStdTx(f, t string, coins []types.Coin, memo string) (tx.StdTx, error) {
	...
	second, err := types.AccAddressFromBech32(mayachain.GetRandomBNBAddress().String())
	if err != nil {
		return tx.StdTx{}, err
	}
...
func (s *BlockScannerTestSuite) TestFromStdTx(c *C) {
	...
	c.Assert(err, IsNil)
	txhash := mayachain.GetRandomTxHash().String()
