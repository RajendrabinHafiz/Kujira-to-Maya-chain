...
func (s *AvalancheSuite) SetUpTest(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case mayaclient.PubKeysEndpoint:
			...
			_, err = rw.Write(buf)
			c.Assert(err, IsNil)
		case mayaclient.InboundAddressesEndpoint:
		...
		case mayaclient.LastBlockEndpoint:
			httpTestHandler(c, rw, "../../../../test/fixtures/endpoints/lastblock/root.json")
		case mayaclient.NodeAccountEndpoint:
...
func (s *AvalancheSuite) TestNewClient(c *C) {
	...
	c.Assert(err, IsNil)
	poolMgr := mayaclient.NewPoolMgr(s.bridge)
...
func (s *AvalancheSuite) TestClient(c *C) {
    ...
    "tx_array": [
        {
            "vault_pub_key": "tmayapub1addwnpepq2mza4j4vplyjw295pkq8j2dan627lz6vufeu22pjx5vnnyjted5vgs8xc7",
            "chain": "AVAX",
