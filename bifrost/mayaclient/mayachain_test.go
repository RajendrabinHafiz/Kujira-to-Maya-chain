...
var _ = Suite(&MayachainSuite{})

func (s *MayachainSuite) SetUpTest(c *C) {
	...
	s.server = httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case strings.HasPrefix(req.RequestURI, MayachainConstants):
		...
		case strings.HasPrefix(req.RequestURI, "/mayachain/mayaname/"):
			httpTestHandler(c, rw, "../../test/fixtures/endpoints/mayaname/mayaname.json")
		}
	}))
...
func (s *MayachainSuite) TearDownTest(c *C) {
	s.server.Close()
}

func (s *MayachainSuite) TestGetThorChainURL(c *C) {
	uri := s.bridge.getMayaChainURL("")
...
func (s *MayachainSuite) TestGet(c *C) {
...
func (s *MayachainSuite) TestGetObservationStdTx_OutboundShouldHaveNotConfirmationCounting(c *C) {
...
func (s *MayachainSuite) TestSign(c *C) {
	pk := stypes.GetRandomPubKey()
	vaultAddr, err := pk.GetAddress(common.BNBChain)
	c.Assert(err, IsNil)
	tx := stypes.NewObservedTx(
...
func (MayachainSuite) TestNewMayachainBridge(c *C) {
	testFunc := func(cfg config.BifrostClientConfiguration, errChecker, sbChecker Checker) {
		...
		c.Assert(err, IsNil)
		sb, err := NewMayachainBridge(cfg, m, NewKeysWithKeybase(kb, cfg.SignerName, cfg.SignerPasswd))
...
func (s *MayachainSuite) TestGetAccountNumberAndSequenceNumber_Success(c *C) {
...
func (s *MayachainSuite) TestGetAccountNumberAndSequenceNumber_Fail(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *MayachainSuite) TestGetAccountNumberAndSequenceNumber_Fail_500(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *MayachainSuite) TestGetAccountNumberAndSequenceNumber_Fail_Unmarshal(c *C) {
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *MayachainSuite) TestEnsureNodeWhitelisted_Success(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
	s.nodeAccountFixture = "../../test/fixtures/endpoints/nodeaccount/template.json"
...
func (s *MayachainSuite) TestEnsureNodeWhitelisted_Fail(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
...
func (s *MayachainSuite) TestGetKeysignParty(c *C) {
...
func (s *MayachainSuite) TestIsCatchingUp(c *C) {
	ok, err := s.bridge.IsCatchingUp()
	c.Assert(err, IsNil)
	c.Assert(ok, Equals, false)
}

func (s *MayachainSuite) TestGetAsgards(c *C) {
	vaults, err := s.bridge.GetAsgards()
	c.Assert(err, IsNil)
	c.Assert(vaults, NotNil)
}

func (s *MayachainSuite) TestGetPubKeys(c *C) {
	pks, err := s.bridge.GetPubKeys()
	c.Assert(err, IsNil)
	c.Assert(pks, HasLen, 6)
}

func (s *MayachainSuite) TestPostNetworkFee(c *C) {
	s.authAccountFixture = "../../test/fixtures/endpoints/auth/accounts/template.json"
...
func (s *MayachainSuite) TestGetConstants(c *C) {
	result, err := s.bridge.GetConstants()
	c.Assert(err, IsNil)
	c.Assert(result, NotNil)
}

func (s *MayachainSuite) TestGetRagnarok(c *C) {
	result, err := s.bridge.RagnarokInProgress()
	c.Assert(err, IsNil)
	c.Assert(result, Equals, false)
}

func (s *MayachainSuite) TestGetMayachainVersion(c *C) {
	result, err := s.bridge.GetMayachainVersion()
	c.Assert(err, IsNil)
	c.Assert(result.EQ(semver.MustParse("0.11.0")), Equals, true)
}

func (s *MayachainSuite) TestGetMimir(c *C) {
	result, err := s.bridge.GetMimir("HaltBNBChain")
	c.Assert(err, IsNil)
	c.Assert(result, Equals, int64(10))
}

func (s *MayachainSuite) TestGetContractAddress(c *C) {
...
func (s *MayachainSuite) TestMAYAName(c *C) {
	result, err := s.bridge.GetMAYAName("test1")
	c.Assert(err, IsNil)
	c.Assert(result.Name, Equals, "test1")
	c.Assert(result.ExpireBlockHeight, Equals, int64(10000))
	c.Assert(result.Aliases, HasLen, 1)
	c.Assert(result.Aliases[0].Chain, Equals, common.BASEChain)
	c.Assert(result.Aliases[0].Address, Equals, common.Address("tmaya1tdfqy34uptx207scymqsy4k5uzfmry5sffuam7"))
}
