...
func (s *ObserverSuite) SetUpSuite(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		if strings.HasPrefix(req.RequestURI, "/errata") { // nolint
			...
			c.Assert(err, IsNil)
		} else if strings.HasPrefix(req.RequestURI, mayaclient.MimirEndpoint) {
			...
			_, err := rw.Write([]byte(`[{ "chain": "BNB", "lastobservedin": 0, "lastsignedout": 0, "mayachain": 0 }]`))
			c.Assert(err, IsNil)
		} else if strings.HasPrefix(req.RequestURI, "/mayachain/lastblock") {
			_, err := rw.Write([]byte(`[{ "chain": "BASEChain", "lastobservedin": 0, "lastsignedout": 0, "mayachain": 0 }]`))
			c.Assert(err, IsNil)
			...
			_, err := rw.Write([]byte(`{ "jsonrpc": "2.0", "id": "", "result": { "asgard": ["tmayapub1addwnpepq2jgpsw2lalzuk7sgtmyakj7l6890f5cfpwjyfp8k4y4t7cw2vk8vvgpt43"], "yggdrasil": ["tmayapub1addwnpepqdqvd4r84lq9m54m5kk9sf4k6kdgavvch723pcgadulxd6ey9u70kujkcf2"] } }`))
			c.Assert(err, IsNil)
			...
			_, err := rw.Write([]byte(`[
  "tmayapub1addwnpepqflvfv08t6qt95lmttd6wpf3ss8wx63e9vf6fvyuj2yy6nnyna5769e5dlr",
  "tmayapub1addwnpepq2flfr96skc5lkwdv0n5xjsnhmuju20x3zndgu42zd8dtkrud9m2vmzp38f",
  "tmayapub1addwnpepqwhnus6xs4208d4ynm05lv493amz3fexfjfx4vptntedd7k0ajlcu40le0r"
]`))
			c.Assert(err, IsNil)

		} else if req.RequestURI == "/mayachain/mayaname/all-good" {
			_, err := rw.Write([]byte(`{
				"name": "all-good",
				"expire_block_height": 10000,
				"owner": "tmaya1tdfqy34uptx207scymqsy4k5uzfmry5sffuam7",
				"aliases": [
					{"chain": "BNB", "address": "tbnb1yycn4mh6ffwpjf584t8lpp7c27ghu03gpvqkfjw"}
				]
			}`))
			c.Assert(err, IsNil)
		} else if req.RequestURI == "/mayachain/mayaname/unknown" {
			_, err := rw.Write([]byte(`{"error": "internal"}`))
			c.Assert(err, IsNil)
		} else if req.RequestURI == "/mayachain/mayaname/bnb-memo" {
			_, err := rw.Write([]byte(`{
				"name": "bnb-memo",
				"expire_block_height": 10000,
				"owner": "tmaya1tdfqy34uptx207scymqsy4k5uzfmry5sffuam7",
...
func (s *ObserverSuite) TestProcess(c *C) {
	...
	metric, err := s.m.GetCounterVec(metrics.ObserverError).GetMetricWithLabelValues("fail_to_send_to_mayachain", "1")
	c.Assert(err, IsNil)
...
func (s *ObserverSuite) TestErrataTx(c *C) {
	...
	c.Assert(obs, NotNil)
	c.Assert(err, IsNil)
	c.Assert(obs.sendErrataTxToMayachain(25, mayachain.GetRandomTxHash(), common.BNBChain), IsNil)
}
...
func (s *ObserverSuite) TestFilterMemoFlag(c *C) {
	...
	result := obs.filterBinanceMemoFlag(common.BNBChain, []types.TxInItem{
		{
			...
			Sender:      mayachain.GetRandomBNBAddress().String(),
			To:          mayachain.GetRandomBNBAddress().String(),
			Coins: common.Coins{
				common.NewCoin(common.BNBAsset, cosmos.NewUint(1024)),
			},
			Gas:                 nil,
			ObservedVaultPubKey: mayachain.GetRandomPubKey(),
		},
	})
	...
	result = obs.filterBinanceMemoFlag(common.BNBChain, []types.TxInItem{
		{
			BlockHeight: 1024,
			Tx:          "tx1",
			Memo:        "swap:BNB.RUNE-67C:" + mayachain.GetRandomBNBAddress().String(),
	...
	// mayaname resolves to a good bnb address, should pass filter
	result = obs.filterBinanceMemoFlag(common.BNBChain, []types.TxInItem{
		{
			BlockHeight: 1024,
			Tx:          "tx1",
	...
	// mayaname is not known, should pass filter
	result = obs.filterBinanceMemoFlag(common.BNBChain, []types.TxInItem{
		{
			BlockHeight: 1024,
			Tx:          "tx1",
	...
	// mayaname resolves to a bnb addr that requires a memo, should be filtered
	result = obs.filterBinanceMemoFlag(common.BNBChain, []types.TxInItem{
		{
			BlockHeight: 1024,
			Tx:          "tx1",
			Memo:        "swap:BNB.LOK-3C0:bnb-memo",
			Sender:      mayachain.GetRandomBNBAddress().String(),
			To:          mayachain.GetRandomBNBAddress().String(),
			Coins: common.Coins{
				common.NewCoin(common.BNBAsset, cosmos.NewUint(1024)),
