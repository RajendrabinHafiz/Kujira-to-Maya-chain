...
func NewPubKeyManager(bridge mayaclient.MayachainBridge, m *metrics.Metrics) (*PubKeyManager, error) {
...
func (pkm *PubKeyManager) updateContractAddresses(pairs []mayaclient.PubKeyContractAddressPair) {
	pkm.rwMutex.Lock()
...
func (pkm *PubKeyManager) fetchPubKeys() {
	...
	if err != nil {
		pkm.logger.Error().Err(err).Msg("fail to get pubkeys from BASEChain")
		return
	}
...
// getPubkeys from BASEChain
func (pkm *PubKeyManager) getPubkeys() ([]mayaclient.PubKeyContractAddressPair, error) {
