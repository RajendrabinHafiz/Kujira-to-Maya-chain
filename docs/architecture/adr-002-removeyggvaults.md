...
There are two types of vaults in BASEChain:

- cold, inbound vaults "asgard" using TSS
- hot, outbound vaults "yggdrasil" using 1of1

This is primarily a result of TSS limitations. During the initial build of BASEChain it was found that TSS's quadratic scaling problem (signing times increase quadratically with the size of member committee) would set a limit on the number of outbounds per second.

Key extract from [TSS Benchmark 2020] (https://github.com/thorchain/Resources/blob/master/Whitepapers/BASEChain-TSS-Benchmark-July2020.pdf)

![image.png](./image.png)

Thus each node retains a 1of1 key to fast-sign outbounds. If they fail to sign, they are slashed and the tx re-delegated to an Asgard. BASEChain with 100 nodes can do 100 outbounds a sec due to 100 ygg vaults. The vault funds are secured via economic security, but the following are outstanding (managed) problems:
...
** Vault Costs **
Ygg funding is one of the larger expenses that increase BASEChain's vault management costs:
...
- https://github.com/thorchain/Resources/blob/master/Whitepapers/BASEChain-TSS-Benchmark-July2020.pdf
