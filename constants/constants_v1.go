...
func NewConstantValue010() *ConstantVals {
	return &ConstantVals{
		int64values: map[ConstantName]int64{
			...
			StagedPoolCost:                     10_00000000,      // amount of cacao to take from a staged pool on every pool cycle
			KillSwitchStart:                    0,                // block height to start the kill switch of BEP2/ERC20 old RUNE
			KillSwitchDuration:                 0,                // number of blocks until swith no longer works
			MinCacaoPoolDepth:                  10000_00000000,   // minimum cacao pool depth to be an available pool
			...
			MinimumBondInCacao:                 10_000_00000000,  // 10k cacao
			...
			PauseOnSlashThreshold:              100_00000000,     // number of cacao to pause the network on the event a vault is slash for theft
			...
			TNSRegisterFee:                     10_00000000,      // registration fee for new MAYAName
			TNSFeeOnSale:                       1000,             // fee for TNS sale in basis points
			TNSFeePerBlock:                     20,               // per block cost for TNS, in cacao
			...
			MinCacaoForMayaFundDist:            100000000,        // The minimum amount of tokens needed to distribute on MayaFund
			...
			POLMaxPoolMovement:                 100,              // Maximum amount of cacao to enter/exit a pool per iteration. This is in basis points of the pool cacao depth
