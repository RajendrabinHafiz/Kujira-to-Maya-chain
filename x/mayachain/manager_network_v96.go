...
func (vm *NetworkMgrV96) UpdateNetwork(ctx cosmos.Context, constAccessor constants.ConstantValues, gasManager GasManager, eventMgr EventManager) error {
	...
	bondReward, bondShare := vm.calcBlockRewards(ctx, totalProvidedLiquidity, totalBonded, totalLiquidityFees)
	...
	evtPools, err := vm.deductPoolRewardDeficit(ctx, pools, totalLiquidityFees, bondReward)
	if err != nil {
		return err
	}

	if !bondReward.IsZero() {
		coin := common.NewCoin(common.BaseAsset(), bondReward)
		if err := vm.k.SendFromModuleToModule(ctx, ReserveName, BondName, common.NewCoins(coin)); err != nil {
			ctx.Logger().Error("fail to transfer funds from reserve to bond", "error", err)
			return fmt.Errorf("fail to transfer funds from reserve to bond: %w", err)
		}
...
func (vm *NetworkMgrV96) calcBlockRewards(ctx cosmos.Context, totalProvidedLiquidity, totalBonded, totalLiquidityFees cosmos.Uint) (cosmos.Uint, cosmos.Uint) {
