...
func (h UnBondHandler) validateV88(ctx cosmos.Context, msg MsgUnBond) error {
	if err := msg.ValidateBasicV88(); err != nil {
		return err
	}
...
func (h UnBondHandler) handleV92(ctx cosmos.Context, msg MsgUnBond) error {
	...
	if msg.BondAddress.Equals(na.BondAddress) && !msg.BondProviderAddress.Empty() {
		if err := refundBond(ctx, msg.TxIn, from, common.EmptyAsset, cosmos.ZeroUint(), &na, h.mgr); err != nil {
		...
		if bondAddr.Equals(msg.BondProviderAddress) {
			bp, err := h.mgr.Keeper().GetBondProviders(ctx, from)
			if err != nil {
				return ErrInternal(err, fmt.Sprintf("fail to get bond providers(%s)", msg.NodeAddress))
			}
			...
			for _, provider := range bp.Providers {
				...
				if err = removeBondAddress(ctx, h.mgr, common.Address(provider.BondAddress.String())); err != nil {
					return ErrInternal(err, "fail to remove bond address")
				}
			...
			if err = removeBondAddress(ctx, h.mgr, common.Address(msg.BondProviderAddress.String())); err != nil {
				return ErrInternal(err, "fail to remove bond address")
			}
		...
		if err = removeBondAddress(ctx, h.mgr, common.Address(from.String())); err != nil {
			return ErrInternal(err, "fail to remove bond address")
		}
	}
