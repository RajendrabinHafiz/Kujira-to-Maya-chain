...
// if an operator of BASEChain node would like to leave and get their bond back , they have to
...
func (h LeaveHandler) handleV76(ctx cosmos.Context, msg MsgLeave) error {
	...
	coin := msg.Tx.Coins.GetCoin(common.BaseAsset())
	if !coin.IsEmpty() {
		err = h.mgr.Keeper().SendFromModuleToAccount(ctx, BondName, nodeAcc.NodeAddress, common.NewCoins(coin))
		if err != nil {
			return ErrInternal(err, fmt.Sprintf("fail to send coins to node account %s", nodeAcc.NodeAddress.String()))
		}
	}
	...
	if nodeAcc.Status == NodeActive {
		...
		if !isMemberOfRetiringVault {
			...
			if !h.mgr.Keeper().VaultExists(ctx, nodeAcc.PubKeySet.Secp256k1) {
				if err := refundBond(ctx, msg.Tx, bondAddr, common.EmptyAsset, cosmos.ZeroUint(), &nodeAcc, h.mgr); err != nil {
					return ErrInternal(err, "fail to refund bond")
				}
