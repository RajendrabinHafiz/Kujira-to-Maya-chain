...
func (h ErrataTxHandler) handleV58(ctx cosmos.Context, msg MsgErrataTx) (*cosmos.Result, error) {
	...
	memo, _ := ParseMemoWithMAYANames(ctx, h.mgr.Keeper(), tx.Memo)
	...
	if runeCoin.Amount.GT(pool.BalanceCacao) {
		runeCoin.Amount = pool.BalanceCacao
	}
	if assetCoin.Amount.GT(pool.BalanceAsset) {
		assetCoin.Amount = pool.BalanceAsset
	}
	pool.BalanceCacao = common.SafeSub(pool.BalanceCacao, runeCoin.Amount)
