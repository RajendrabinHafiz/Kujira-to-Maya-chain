...
// LPs that come from the relationship tree of node < bond providers < LPs.
func (s *SlasherV92) SlashNodeAccountLP(ctx cosmos.Context, na NodeAccount, slash cosmos.Uint) (cosmos.Uint, []types.PoolAmt, error) {
	...
	for _, b := range bp.Providers {
		...
		if err != nil {
			return totalSlashed, slashedAmountsPerPool, ErrInternal(err, fmt.Sprintf("fail to get lps for bond provider: %s", b.BondAddress))
		}
