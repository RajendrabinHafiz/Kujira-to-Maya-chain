...
func (h WithdrawLiquidityHandler) validateV96(ctx cosmos.Context, msg MsgWithdrawLiquidity) error {
	...
	if !msg.Tx.Chain.Equals(common.BASEChain) && msg.WithdrawAddress.IsChain(common.BASEChain) && !msg.Tx.ID.Equals(common.BlankTxID) {
		// asymmetric LP trying symetric withdraw from external chain
...
func (h WithdrawLiquidityHandler) handleV95(ctx cosmos.Context, msg MsgWithdrawLiquidity) (*cosmos.Result, error) {
	...
	if !assetAmt.IsZero() {
		...
		if !msg.Asset.IsNativeBase() && !lp.AssetAddress.IsChain(msg.Asset.Chain) {
