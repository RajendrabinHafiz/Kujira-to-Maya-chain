...
func (m Pool) String() string {
	sb := strings.Builder{}
	sb.WriteString(fmt.Sprintln("rune-balance: " + m.BalanceCacao.String()))
	...
	sb.WriteString(fmt.Sprintln("pending-inbound-rune: " + m.PendingInboundCacao.String()))
...
func (m Pool) AssetValueInRune(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceCacao.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
	return common.GetUncappedShare(m.BalanceCacao, m.BalanceAsset, amt)
}
...
func (m Pool) RuneReimbursementForAssetWithdrawal(amt cosmos.Uint) cosmos.Uint {
	...
	return common.GetUncappedShare(m.BalanceCacao, denom, amt)
}
...
func (m Pool) RuneValueInAsset(amt cosmos.Uint) cosmos.Uint {
	...
	assetAmt := common.GetUncappedShare(m.BalanceAsset, m.BalanceCacao, amt)
...
// amount is withdrawn from the pool, the constant product of depths rule is
// preserved.
func (m Pool) AssetDisbursementForCacaoAdd(amt cosmos.Uint) cosmos.Uint {
	if m.BalanceCacao.IsZero() || m.BalanceAsset.IsZero() {
		return cosmos.ZeroUint()
	}
	denom := m.BalanceCacao.Add(amt)
...
func (m Pool) GetLUVI() cosmos.Uint {
	bigInt := &big.Int{}
	balRune := m.BalanceCacao.MulUint64(1e12).BigInt()
