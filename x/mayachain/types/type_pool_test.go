...
func (PoolTestSuite) TestPool(c *C) {
	...
	p1.BalanceCacao = cosmos.NewUint(100 * common.One)
...
func (PoolTestSuite) TestPools(c *C) {
	...
	btc.BalanceCacao = cosmos.NewUint(10)
	...
	c.Check(pool.BalanceCacao.Uint64(), Equals, uint64(10))

	pool.BalanceCacao = cosmos.NewUint(20)
	pools = pools.Set(pool)
	pool, ok = pools.Get(common.BTCAsset)
	c.Check(ok, Equals, true)
	c.Check(pool.Asset.Equals(common.BTCAsset), Equals, true)
	c.Check(pool.BalanceCacao.Uint64(), Equals, uint64(20))
}
...
func (PoolTestSuite) TestCalcUnits(c *C) {
	...
	pool.BalanceCacao = cosmos.NewUint(100)
	pool.LPUnits = cosmos.NewUint(100)
	...
	pool.BalanceCacao = cosmos.NewUint(1_000)
	synthSupply = cosmos.NewUint(100)
	units = pool.CalcUnits(version, synthSupply)
...
func (PoolTestSuite) TestReimbursementAndDisbursement(c *C) {
	...
	c.Check(p.AssetDisbursementForCacaoAdd(cosmos.NewUint(900*common.One)).Equal(cosmos.NewUint(45*common.One)), Equals, true)

	p.BalanceCacao = cosmos.NewUint(0)
	c.Check(p.RuneDisbursementForAssetAdd(cosmos.NewUint(1*common.One)).Equal(cosmos.NewUint(0*common.One)), Equals, true)
	c.Check(p.AssetDisbursementForCacaoAdd(cosmos.NewUint(1*common.One)).Equal(cosmos.NewUint(0*common.One)), Equals, true)
...
func (PoolTestSuite) TestLUVI(c *C) {
	p := NewPool()
	p.BalanceCacao = cosmos.NewUint(100)
