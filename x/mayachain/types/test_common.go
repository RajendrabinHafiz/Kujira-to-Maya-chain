...
func GetRandomTxOutItem() TxOutItem {
	return TxOutItem{
		...
		Coin:        common.NewCoin(common.BNBAsset, cosmos.NewUint(100000)),
		Memo:        "OUT:xyz",
		MaxGas:      common.Gas{common.NewCoin(common.BNBAsset, cosmos.NewUint(100))},
...
func GetRandomAZTECAddress() common.Address {
	name := common.RandStringBytesMask(10)
	str, _ := common.ConvertAndEncode("aztec", crypto.AddressHash([]byte(name)))
	aztec, _ := common.NewAddress(str)
	return aztec
}
...
func GetRandomDASHAddress() common.Address {
	pubKey := GetRandomPubKey()
	addr, _ := pubKey.GetAddress(common.DASHChain)
	return addr
}
...
func SetupConfigForTest() {
	...
	config.SetCoinType(cmd.BASEChainCoinType)
	config.SetPurpose(cmd.BASEChainCoinPurpose)
