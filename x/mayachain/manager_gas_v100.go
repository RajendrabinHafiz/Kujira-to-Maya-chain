...
// GasMgrV100 implement GasManager interface which will store the gas related events happened in thorchain to memory
...
// newGasMgrV100 create a new instance of GasMgrV100
func newGasMgrV100(constantsAccessor constants.ConstantValues, k keeper.Keeper) *GasMgrV100 {
	return &GasMgrV100{
...
func (gm *GasMgrV100) GetFee(ctx cosmos.Context, chain common.Chain, asset common.Asset) cosmos.Uint {
	...
	// if the asset is Native RUNE or CACAO , then we could just return the transaction Fee
	// because transaction fee is always in native CACA/RUNE. This is called from both
	// ends when a tx is sent out to THORChain so we need to check tha MAYA.CACAO-THORChain case
	if asset.IsBase() && chain.Equals(common.BASEChain) ||
		asset.IsBase() && chain.Equals(common.THORChain) ||
		asset.Equals(common.RUNEAsset) && chain.Equals(common.THORChain) {
...
func (gm *GasMgrV100) getRuneInAssetValue(ctx cosmos.Context, transactionFee cosmos.Uint, asset common.Asset) cosmos.Uint {
	if asset.IsSyntheticAsset() {
...
func (gm *GasMgrV100) GetGasRate(ctx cosmos.Context, chain common.Chain) cosmos.Uint {
	...
	transactionFee := cosmos.NewUint(uint64(outboundTxFee))
	if chain.Equals(common.BASEChain) || chain.Equals(common.THORChain) {
		return transactionFee
	}
	networkFee, err := gm.keeper.GetNetworkFee(ctx, chain)
	if err != nil {
