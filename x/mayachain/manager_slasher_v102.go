...
// newSlasherV102 create a new instance of Slasher
func newSlasherV102(keeper keeper.Keeper, eventMgr EventManager) *SlasherV102 {
	return &SlasherV102{keeper: keeper, eventMgr: eventMgr}
}
...
func (s *SlasherV102) slashNotObserving(ctx cosmos.Context, txHash common.TxID, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV102) checkSignerAndSlash(ctx cosmos.Context, nodes NodeAccounts, blockHeight int64, signers []cosmos.AccAddress, constAccessor constants.ConstantValues) {
...
func (s *SlasherV102) SlashNodeAccountLP(ctx cosmos.Context, na NodeAccount, slash cosmos.Uint) (cosmos.Uint, []types.PoolAmt, error) {
	...
	for _, b := range bp.Providers {
		...
		// Slash correspoding lpunits proportionally to LP.
		for _, lp := range lps {
			...
			if pool.IsAvailable() {
				...
				slashLPUnits := common.GetSafeShare(slash, naBond, lp.Units)
...
func (s *SlasherV102) needsNewVault(ctx cosmos.Context, mgr Manager, nas int, signingTransPeriod, startHeight int64, inhash common.TxID, pk common.PubKey) bool {
