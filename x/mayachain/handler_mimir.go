...
var mimirValidKeyV95 = regexp.MustCompile(constants.MimirKeyRegex).MatchString
...
func (h MimirHandler) handleV92(ctx cosmos.Context, msg MsgMimir) error {
	...
	if h.isAdmin(msg.Signer) {
		...
		if msg.Value < 0 {
			...
			if strings.ToUpper(msg.Key) == "IBCRECEIVEENABLED" || strings.ToUpper(msg.Key) == "IBCSENDENABLED" {
				h.setIBCTransfer(ctx, msg.Key, msg.Value)
			}
		}
...
func (h MimirHandler) setIBCTransfer(ctx cosmos.Context, key string, value int64) {
	params := h.mgr.Keeper().GetIBCTransferParams(ctx)
	if key == strings.ToUpper("IBCReceiveEnabled") {
		if value == 1 {
			params.ReceiveEnabled = true
		} else if value == 0 {
			params.ReceiveEnabled = false
		}
		h.mgr.Keeper().SetIBCTransferParams(ctx, params)
	} else if key == strings.ToUpper("IBCSendEnabled") {
		if value == 1 {
