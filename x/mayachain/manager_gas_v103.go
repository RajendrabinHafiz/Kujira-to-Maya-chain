...
// GasMgrV103 implement GasManager interface which will store the gas related events happened in thorchain to memory
...
// newGasMgrV103 create a new instance of GasMgrV103
func newGasMgrV103(constantsAccessor constants.ConstantValues, k keeper.Keeper) *GasMgrV103 {
	return &GasMgrV103{
...
func (gm *GasMgrV103) GetFee(ctx cosmos.Context, chain common.Chain, asset common.Asset) cosmos.Uint {
	...
	// ends when a tx is sent out to BASEChain so we need to check tha MAYA.CACAO/THORChain case
...
func (gm *GasMgrV103) getRuneInAssetValue(ctx cosmos.Context, transactionFee cosmos.Uint, asset common.Asset) cosmos.Uint {
	if asset.IsSyntheticAsset() {
