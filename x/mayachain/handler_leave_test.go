...
import (
	...
	kvTypes "gitlab.com/mayachain/mayanode/x/mayachain/keeper/types"
	. "gopkg.in/check.v1"
...
func (HandlerLeaveSuite) TestLeaveHandler_NotActiveNodeLeave(c *C) {
	...
	leaveHandler := NewLeaveHandler(mgr)
	acc2 := GetRandomValidatorNode(NodeStandby)
	SetupLiquidityBondForTest(c, w.ctx, w.keeper, common.BTCAsset, acc2.BondAddress, acc2, cosmos.NewUint(100*common.One))
	FundModule(c, w.ctx, w.keeper, BondName, 100*common.One)
	c.Assert(w.keeper.SetNodeAccount(w.ctx, acc2), IsNil)
	ygg := NewVault(w.ctx.BlockHeight(), ActiveVault, YggdrasilVault, acc2.PubKeySet.Secp256k1, common.Chains{common.BaseAsset().Chain}.Strings(), []ChainContract{})
	...
	tx := common.NewTx(
		...
		acc2.BondAddress,
		GetRandomBNBAddress(),
		common.Coins{common.NewCoin(common.BaseAsset(), cosmos.OneUint())},
		BNBGasFeeSingleton,
		"LEAVE",
	)
...
func (HandlerLeaveSuite) TestLeaveHandler_ActiveNodeLeave(c *C) {
	...
	SetupLiquidityBondForTest(c, w.ctx, w.keeper, common.BTCAsset, acc2.BondAddress, acc2, cosmos.NewUint(100*common.One))
	FundModule(c, w.ctx, w.keeper, BondName, 100*common.One)
	bp := NewBondProviders(acc2.NodeAddress)
	acc, err := acc2.BondAddress.AccAddress()
	c.Assert(err, IsNil)
	bp.Providers = []BondProvider{NewBondProvider(acc)}
	bp.Providers[0].Bonded = true
	c.Assert(w.keeper.SetBondProviders(w.ctx, bp), IsNil)
	c.Assert(w.keeper.SetNodeAccount(w.ctx, acc2), IsNil)
	txID := GetRandomTxHash()
	tx := common.NewTx(
		txID,
	...
	c.Assert(err, IsNil)
	acc2Bond, err := w.keeper.CalcNodeLiquidityBond(w.ctx, acc2)
	c.Assert(err, IsNil)
	c.Check(acc2Bond.Equal(cosmos.NewUint(20000000000)), Equals, true, Commentf("Bond:%d\n", acc2Bond.Uint64()))
}
...
func (HandlerLeaveSuite) TestLeaveDifferentValidations(c *C) {
	testCases := []struct {
		...
		{
			name: "fail to refund rewards should return an error",
			messageProvider: func(ctx cosmos.Context, helper *LeaveHandlerTestHelper) cosmos.Msg {
				nodeAccount := GetRandomValidatorNode(NodeStandby)
				activeNodeAccount := GetRandomValidatorNode(NodeActive)
				SetupLiquidityBondForTest(c, ctx, helper.Keeper, common.BTCAsset, nodeAccount.BondAddress, nodeAccount, cosmos.NewUint(100*common.One))
				...
				bp.Providers[0].Bonded = true
				nodeAccount.Reward = cosmos.NewUint(100 * common.One)
		...
		{
			name: "when node account is still belongs to a retiring vault , don't unlock bond",
			messageProvider: func(ctx cosmos.Context, helper *LeaveHandlerTestHelper) cosmos.Msg {
				...
				bp.Providers = append(bp.Providers, NewBondProvider(acc))
				bp.Providers[0].Bonded = true
				SetupLiquidityBondForTest(c, ctx, helper.Keeper, common.BNBAsset, nodeAccount.BondAddress, nodeAccount, cosmos.NewUint(100))
				c.Assert(helper.Keeper.SetBondProviders(ctx, bp), IsNil)
				activeNodeAccount := GetRandomValidatorNode(NodeActive)
				c.Assert(helper.Keeper.SetNodeAccount(ctx, activeNodeAccount), IsNil)
				c.Assert(helper.Keeper.SetNodeAccount(ctx, nodeAccount), IsNil)
			...
			validator: func(c *C, ctx cosmos.Context, result *cosmos.Result, err2 error, helper *LeaveHandlerTestHelper, name string, msg cosmos.Msg) { // nolint
				...
				c.Assert(err, IsNil)
				naBond, err := helper.Keeper.CalcNodeLiquidityBond(ctx, na)
				c.Assert(err, IsNil)
				c.Assert(naBond.Equal(cosmos.NewUint(200)), Equals, true)
				c.Check(err, IsNil, Commentf(name))
				c.Check(result, NotNil, Commentf(name))
			},
		},
