...
func (k KVStore) ListValidatorsWithBond(ctx cosmos.Context) (NodeAccounts, error) {
	...
	for ; naIterator.Valid(); naIterator.Next() {
		...
		naBond, err := k.CalcNodeLiquidityBond(ctx, na)
		if err != nil {
			return nodeAccounts, err
		}
		if na.Type == NodeTypeValidator && !naBond.IsZero() {
...
// EnsureAztecAddressUnique check the given aztec bech32 address set against all the the node account
// return an error when it is overlap with any existing one
func (k KVStore) EnsureAztecAddressUnique(ctx cosmos.Context, aztecAddress common.Address) error {
	iter := k.GetNodeAccountIterator(ctx)
	defer iter.Close()
	for ; iter.Valid(); iter.Next() {
		var na NodeAccount
		...
		if strings.EqualFold("", aztecAddress.String()) {
			return dbError(ctx, "", errors.New("Aztec Address cannot be empty"))
		}
		if na.AztecAddress == aztecAddress {
			return dbError(ctx, "", fmt.Errorf("%s already exist", na.AztecAddress))
		}
	}
...
func (k KVStore) DecNodeAccountSlashPoints(ctx cosmos.Context, addr cosmos.AccAddress, pts int64) error {
	...
	metricLabels, _ := ctx.Context().Value(constants.CtxMetricLabels).([]metrics.Label)
	telemetry.IncrCounterWithLabels(
		[]string{"mayanode", "point_slash_refund"},
		float32(dec),
		append(
			metricLabels,
			telemetry.NewLabel("address", addr.String()),
...
// CalcNodeLiquidityBond takes the liquidity provided by every bp of a NodeAccount and calculates its rune side value based on LP Units
func (k KVStore) CalcNodeLiquidityBond(ctx cosmos.Context, na NodeAccount) (cosmos.Uint, error) {
	nodeBond := cosmos.ZeroUint()
	...
	for _, b := range bp.Providers {
		...
		bond, err := k.CalcLPLiquidityBond(ctx, common.Address(b.BondAddress.String()), na.NodeAddress)
		if err != nil {
			return nodeBond, err
		}
		nodeBond = nodeBond.Add(bond)
	}
	return nodeBond, nil
}
