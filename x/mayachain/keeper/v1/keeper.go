...
const (
	...
	prefixForgiveSlashVoter       kvTypes.DbPrefix = "forgive_slash/"
	...
	prefixMAYAName                kvTypes.DbPrefix = "mayaname/"
	prefixRollingPoolLiquidityFee kvTypes.DbPrefix = "rolling_pool_liquidity_fee/"
	prefixLiquidityAuctionTier    kvTypes.DbPrefix = "la_tier/"
...
func NewKVStore(cdc codec.BinaryCodec, coinKeeper bankkeeper.Keeper, accountKeeper authkeeper.AccountKeeper, ibcTransferkeeper ibctransferkeeper.Keeper, storeKey cosmos.StoreKey, version semver.Version) KVStore {
	return KVStore{
...
func (k KVStore) GetRuneBalanceOfModule(ctx cosmos.Context, moduleName string) cosmos.Uint {
	return k.GetBalanceOfModule(ctx, moduleName, common.BaseNative.Native())
}
...
func (k KVStore) MintToModule(ctx cosmos.Context, module string, coin common.Coin) error {
	...
	if k.GetVersion().GTE(semver.MustParse("1.95.0")) {
		...
		coin := k.coinKeeper.GetSupply(ctx, common.BaseAsset().Native())
...
func (k KVStore) GetIBCTransferParams(ctx cosmos.Context) ibctransfertypes.Params {
	return k.ibcTransferkeeper.GetParams(ctx)
}

func (k KVStore) SetIBCTransferParams(ctx cosmos.Context, params ibctransfertypes.Params) {
	k.ibcTransferkeeper.SetParams(ctx, params)
}
