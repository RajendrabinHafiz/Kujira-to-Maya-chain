...
func getMsgWithdrawFromMemo(memo WithdrawLiquidityMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	...
	if !fromAddress.GetChain().Equals(common.BASEChain) && !pairAddress.Equals(common.NoAddress) && pairAddress.GetChain().Equals(common.BASEChain) {
...
func getMsgManageMAYANameFromMemo(memo ManageMAYANameMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	...
	return NewMsgManageMAYAName(memo.Name, memo.Chain, memo.Address, tx.Tx.Coins[0], memo.Expire, memo.PreferredAsset, memo.Owner, signer), nil
}
