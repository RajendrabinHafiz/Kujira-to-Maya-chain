...
func GetCmdForgiveSlash() *cobra.Command {
	return &cobra.Command{
		Use:   "forgive_slash [block_amount] [node address (optional)]",
		Short: "votes to forgive slash of node address or to all node addresses",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			amount, err := cosmos.ParseUint(args[0])
			if err != nil {
				return err
			}
			...
			if len(args) > 2 {
				addr, err = cosmos.AccAddressFromBech32(args[2])
				if err != nil {
					return fmt.Errorf("invalid node address: %w", err)
				}
			}

			msg := types.NewMsgForgiveSlash(amount, addr, clientCtx.GetFromAddress())
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
...
func GetCmdSetAztecAddress() *cobra.Command {
	return &cobra.Command{
		Use:   "set-aztec-address [bech32_aztec_chain_address]",
		Short: "set aztec address, the account use to sign this tx has to be whitelist first",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
			...
			fmt.Println(args[0])
			aztecAddress, err := common.NewAddress(args[0])
			if err != nil {
				return fmt.Errorf("fail to parse bech32 address ,err:%w", err)
			}

			msg := types.NewMsgSetAztecAddress(aztecAddress, clientCtx.GetFromAddress())
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
