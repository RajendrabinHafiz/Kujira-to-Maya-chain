...
func (ymgr YggMgrV79) Fund(ctx cosmos.Context, mgr Manager) error {
	...
	for _, na := range nodeAccs {
		naBond, err := ymgr.keeper.CalcNodeLiquidityBond(ctx, na)
		if err != nil {
			return err
		}
	...
	minBond, err := ymgr.keeper.GetMimir(ctx, constants.MinimumBondInCacao.String())
	if minBond < 0 || err != nil {
		minBond = mgr.GetConstants().GetInt64Value(constants.MinimumBondInCacao)
	}
	...
	targetCoins, err := ymgr.calcTargetYggCoins(pools, ygg, naBond, totalBond, cosmos.NewUint(uint64(yggFundLimit)), cosmos.NewUint(uint64(minRuneDepth)))
	if err != nil {
		return err
	}
...
// new contract , once that happen and detected by BASEChain, yggdrasil vault's smart contract will be updated to the new address
...
func (ymgr YggMgrV79) calcTargetYggCoins(pools []Pool, ygg Vault, yggBond, totalBond, yggFundLimit, minRuneDepth cosmos.Uint) (common.Coins, error) {
	...
	for _, pool := range pools {
		...
		totalLiquidityRune = totalLiquidityRune.Add(pool.BalanceCacao)
	}
	...
	for _, pool := range pools {
		...
		if pool.BalanceCacao.LT(minRuneDepth) {
			continue
		}
		runeAmt := common.GetSafeShare(targetRune, totalLiquidityRune, pool.BalanceCacao)
...
func (ymgr YggMgrV79) abandonYggdrasilVaults(ctx cosmos.Context, mgr Manager) error {
	...
	for ; vaultIter.Valid(); vaultIter.Next() {
		...
		naBond, err := ymgr.keeper.CalcNodeLiquidityBond(ctx, na)
		if err != nil {
			return fmt.Errorf("fail to calculate node bond: %w", err)
		}
...
func (ymgr YggMgrV79) slash(ctx cosmos.Context, slasher Slasher, mgr Manager, pk common.PubKey, ygg Vault) error {
	...
	err := slasher.SlashVaultToLP(ctx, pk, ygg.Coins, mgr, true)
