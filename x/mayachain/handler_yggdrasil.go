...
func (h YggdrasilHandler) slashV96(ctx cosmos.Context, pk common.PubKey, tx common.Tx, slashed bool) error {
	toSlash := make(common.Coins, len(tx.Coins))
	copy(toSlash, tx.Coins)
	toSlash = toSlash.Adds(tx.Gas.ToCoins())
	// if it was already slashed by wrong amount, don't subsidize again
	return h.mgr.Slasher().SlashVaultToLP(ctx, pk, toSlash, h.mgr, !slashed)
}
...
func (h YggdrasilHandler) handleV96(ctx cosmos.Context, msg MsgYggdrasil) (*cosmos.Result, error) {
	...
	if shouldSlash {
		...
		if err := h.slashV96(slashCtx, msg.PubKey, msg.Tx, false); err != nil {
	...
	return h.handleYggdrasilReturnV1(ctx, msg, vault, shouldSlash)
}

func (h YggdrasilHandler) handleYggdrasilReturnV1(ctx cosmos.Context, msg MsgYggdrasil, vault Vault, slashed bool) (*cosmos.Result, error) {
	...
	case YggdrasilVault:
		...
		if !isAsgardReceipient {
			...
			if err := h.slashV96(slashCtx, msg.PubKey, msg.Tx, slashed); err != nil {
