...
func ValidateGenesis(data GenesisState) error {
	...
	for _, n := range data.Mayanames {
		if len(n.Name) > 30 {
			return errors.New("MAYAName cannot exceed 30 characters")
		}
		if !IsValidMAYANameV1(n.Name) {
...
func InitGenesis(ctx cosmos.Context, keeper keeper.Keeper, data GenesisState) []abci.ValidatorUpdate {
	...
	for _, n := range data.Mayanames {
		keeper.SetMAYAName(ctx, n)
	}

	// Mint coins into the reserve
	if data.Reserve > 0 {
		coin := common.NewCoin(common.BaseNative, cosmos.NewUint(data.Reserve))
	...
	if data.Asgard > 0 {
		coin := common.NewCoin(common.BaseNative, cosmos.NewUint(data.Asgard))
		if err := keeper.MintToModule(ctx, ModuleName, coin); err != nil {
			panic(err)
		}
	...
	// Mint coins into the maya fund
	if data.MayaFund > 0 {
		coin := common.NewCoin(common.BaseNative, cosmos.NewUint(data.MayaFund))
		if err := keeper.MintToModule(ctx, ModuleName, coin); err != nil {
			panic(err)
		}
		if err := keeper.SendFromModuleToModule(ctx, ModuleName, MayaFund, common.NewCoins(coin)); err != nil {
			panic(err)
		}
	}
	...
	for _, admin := range ADMINS {
		...
		mimir, _ := common.NewAsset("MAYA.MIMIR")
	...
	mayaAddr, _ := keeper.GetModuleAddress(MayaFund)
	ctx.Logger().Info("Reserve Module", "address", mayaAddr.String())
...
func getLiquidityProviders(ctx cosmos.Context, k keeper.Keeper, asset common.Asset) LiquidityProviders {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if lp.Units.IsZero() && lp.PendingCacao.IsZero() && lp.PendingAsset.IsZero() {
			continue
		}
...
func ExportGenesis(ctx cosmos.Context, k keeper.Keeper) GenesisState {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		liquidityBond, err := k.CalcNodeLiquidityBond(ctx, na)
		if err != nil {
			ctx.Logger().Error("fail to get liquiditybond for node account:", na)
		}

		if liquidityBond.IsZero() {
			continue
		}
	...
	iterNames := k.GetMAYANameIterator(ctx)
