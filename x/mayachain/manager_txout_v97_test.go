...
func (s TxOutStoreV97Suite) TestAddGasFees(c *C) {
	...
	mgr.gasMgr = newGasMgrV98(constAccessor, mgr.Keeper())
...
type TestCalcKeeperV97 struct {
	keeper.KVStoreDummy
	value map[int64]cosmos.Uint
	mimir map[string]int64
}

func (k *TestCalcKeeperV97) GetPool(ctx cosmos.Context, asset common.Asset) (types.Pool, error) {
	pool := NewPool()
	pool.Asset = asset
...
func (k *TestCalcKeeperV97) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	return k.mimir[key], nil
}

func (k *TestCalcKeeperV97) GetTxOutValue(ctx cosmos.Context, height int64) (cosmos.Uint, error) {
...
func (s TxOutStoreV97Suite) TestAddOutTxItemInteractionWithPool(c *C) {
	...
	c.Assert(pool.BalanceCacao.Equal(cosmos.NewUint(9999775005)), Equals, true, Commentf("%d", pool.BalanceCacao.Uint64()))
}
