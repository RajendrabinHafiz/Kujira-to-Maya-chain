...
type WithdrawSuiteV105 struct{}

var _ = Suite(&WithdrawSuiteV105{})

type WithdrawTestKeeperV105 struct {
	keeper.KVStoreDummy
...
func getWithdrawTestKeeper2(c *C, ctx cosmos.Context, k keeper.Keeper, runeAddress common.Address, tier int64, withdrawCounter cosmos.Uint) *WithdrawTestKeeperV105 {
...
func NewWithdrawTestKeeperV105(keeper keeper.Keeper) *WithdrawTestKeeperV105 {
	return &WithdrawTestKeeperV105{
		keeper:      keeper,
...
func (k *WithdrawTestKeeperV105) PoolExist(ctx cosmos.Context, asset common.Asset) bool {
	return !asset.Equals(common.Asset{Chain: common.BNBChain, Symbol: "NOTEXIST", Ticker: "NOTEXIST"})
}

func (k *WithdrawTestKeeperV105) GetPool(ctx cosmos.Context, asset common.Asset) (types.Pool, error) {
...
func (k *WithdrawTestKeeperV105) SetPool(ctx cosmos.Context, ps Pool) error {
	k.store[ps.Asset.String()] = ps
	return nil
}

func (k *WithdrawTestKeeperV105) GetGas(ctx cosmos.Context, asset common.Asset) ([]cosmos.Uint, error) {
	return []cosmos.Uint{cosmos.NewUint(37500), cosmos.NewUint(30000)}, nil
}

func (k *WithdrawTestKeeperV105) GetLiquidityProvider(ctx cosmos.Context, asset common.Asset, addr common.Address) (LiquidityProvider, error) {
...
func (k *WithdrawTestKeeperV105) GetNetworkFee(ctx cosmos.Context, chain common.Chain) (NetworkFee, error) {
	return k.networkFees[chain], nil
}

func (k *WithdrawTestKeeperV105) SaveNetworkFee(ctx cosmos.Context, chain common.Chain, networkFee NetworkFee) error {
	k.networkFees[chain] = networkFee
	return nil
}

func (k *WithdrawTestKeeperV105) SetLiquidityProvider(ctx cosmos.Context, lp LiquidityProvider) {
	k.keeper.SetLiquidityProvider(ctx, lp)
}

func (k *WithdrawTestKeeperV105) SetLiquidityAuctionTier(ctx cosmos.Context, addr common.Address, tier int64) error {
	return k.keeper.SetLiquidityAuctionTier(ctx, addr, tier)
}

func (k *WithdrawTestKeeperV105) GetLiquidityAuctionTier(ctx cosmos.Context, addr common.Address) (int64, error) {
	return k.keeper.GetLiquidityAuctionTier(ctx, addr)
}

func (k *WithdrawTestKeeperV105) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	return k.keeper.GetMimir(ctx, key)
}

func (k *WithdrawTestKeeperV105) SetMimir(ctx cosmos.Context, key string, value int64) {
	k.keeper.SetMimir(ctx, key, value)
}

func (s *WithdrawSuiteV105) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func areLpsEqualAssert(c *C, lp1 types.LiquidityProvider, lp2 types.LiquidityProvider) {
	c.Assert(lp1.Asset, Equals, lp2.Asset)
	c.Assert(lp1.AssetAddress, Equals, lp2.AssetAddress)
	c.Assert(lp1.AssetDepositValue, Equals, lp2.AssetDepositValue)
	c.Assert(lp1.LastAddHeight, Equals, lp2.LastAddHeight)
	c.Assert(lp1.LastWithdrawCounterHeight, Equals, lp2.LastWithdrawCounterHeight)
	c.Assert(lp1.LastWithdrawHeight, Equals, lp2.LastWithdrawHeight)
	c.Assert(lp1.NodeBondAddress.Equals(lp2.NodeBondAddress), Equals, true)
	c.Assert(lp1.PendingAsset, Equals, lp2.PendingAsset)
	c.Assert(lp1.PendingCacao, Equals, lp2.PendingCacao)
	c.Assert(lp1.PendingTxID, Equals, lp2.PendingTxID)
	c.Assert(lp1.CacaoAddress, Equals, lp2.CacaoAddress)
	c.Assert(lp1.CacaoDepositValue, Equals, lp2.CacaoDepositValue)
	c.Assert(lp1.Units, Equals, lp2.Units)
	c.Assert(lp1.WithdrawCounter, Equals, lp2.WithdrawCounter)
}

// TestValidateWithdraw is to test validateWithdraw function
func (s WithdrawSuiteV105) TestValidateWithdraw(c *C) {
...
func (s WithdrawSuiteV105) TestCalculateUnsake(c *C) {
	inputs := []struct {
		name                  string
	...
	for _, item := range inputs {
		c.Logf("name:%s", item.name)
		withDrawRune, withDrawAsset, unitAfter, err := calculateWithdrawV102(item.poolUnit, item.poolRune, item.poolAsset, item.lpUnit, cosmos.ZeroUint(), item.percentage, common.EmptyAsset)
...
func (WithdrawSuiteV105) TestWithdraw(c *C) {
	ctx, mgr := setupManagerForTest(c)
	accountAddr := GetRandomValidatorNode(NodeWhiteListed).NodeAddress
	runeAddress := GetRandomBaseAddress()
	ps := NewWithdrawTestKeeperV105(mgr.Keeper())
	ps2 := getWithdrawTestKeeperV105(c, ctx, mgr.Keeper(), runeAddress, 3, cosmos.ZeroUint())
	...
	for _, tc := range testCases {
		...
		r, asset, _, _, _, err := withdrawV105(ctx, tc.msg, mgr)
...
func (WithdrawSuiteV105) TestWithdrawAsym(c *C) {
	accountAddr := GetRandomValidatorNode(NodeWhiteListed).NodeAddress
	runeAddress := GetRandomBaseAddress()
...
func (WithdrawSuiteV105) TestWithdrawPendingCacaoOrAsset(c *C) {
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err := withdrawV105(ctx, msg, mgr)
	c.Assert(err, IsNil)
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err = withdrawV105(ctx, msg1, mgr)
	c.Assert(err, IsNil)
...
func (s *WithdrawSuiteV105) TestWithdrawWithImpermanentLossProtection(c *C) {
	accountAddr := GetRandomValidatorNode(NodeActive).NodeAddress
	ctx, mgr := setupManagerForTest(c)
	pool := Pool{
		BalanceCacao: cosmos.NewUint(100 * common.One),
	...
	cacaoAmt, assetAmt, protectionCacaoAmt, unitsClaimed, gas, err := withdrawV105(newctx, msg2, mgr)
	c.Assert(err, IsNil)
	p, err = mgr.Keeper().GetPool(ctx, common.BTCAsset)
	c.Assert(err, IsNil)
...
func (s *WithdrawSuiteV105) TestWithdrawPendingLiquidityShouldRoundToPoolDecimals(c *C) {
	accountAddr := GetRandomValidatorNode(NodeActive).NodeAddress
	ctx, mgr := setupManagerForTest(c)
	pool := Pool{
		BalanceCacao: cosmos.NewUint(100 * common.One),
	...
	runeAmt, assetAmt, protectoinRuneAmt, unitsClaimed, _, err := withdrawV105(newctx, msg2, mgr)
	c.Assert(err, IsNil)
...
func getWithdrawTestKeeperV105(c *C, ctx cosmos.Context, k keeper.Keeper, runeAddress common.Address, tier int64, withdrawCounter cosmos.Uint) keeper.Keeper {
	store := NewWithdrawTestKeeperV105(k)
	pool := Pool{
		BalanceCacao: cosmos.NewUint(100 * common.One),
		BalanceAsset: cosmos.NewUint(100 * common.One),
...
func (WithdrawSuiteV105) TestCalcImpLossV105(c *C) {
	testCases := []struct {
		name                 string
		pool                 Pool
...
func (s WithdrawSuiteV105) TestAssetToWithdrawV105(c *C) {
	msg := MsgWithdrawLiquidity{
		Asset:           common.BTCAsset,
		WithdrawalAsset: common.BTCAsset,
	}
...
func (WithdrawSuiteV105) TestcheckWithdrawLimit(c *C) {
	ctx, mgr := setupManagerForTest(c)
	...
	for _, tc := range testCases {
		c.Logf("name:%s", tc.name)
		mgr.K = getWithdrawTestKeeperV105(c, ctx, mgr.Keeper(), tc.msg.WithdrawAddress, tc.lpTier, tc.withdrawCounterIn)
...
func (s WithdrawSuiteV105) TestAssetToWithdrawV89(c *C) {
	msg := MsgWithdrawLiquidity{
		Asset:           common.BTCAsset,
		WithdrawalAsset: common.BTCAsset,
	}
...
func (WithdrawSuiteV105) TestWithdrawSynth(c *C) {
	accountAddr := GetRandomValidatorNode(NodeActive).NodeAddress
	ctx, mgr := setupManagerForTest(c)
...
func (WithdrawSuiteV105) TestWithdrawSynthSingleLP(c *C) {
	accountAddr := GetRandomValidatorNode(NodeActive).NodeAddress
	ctx, mgr := setupManagerForTest(c)
	asset := common.BTCAsset.GetSyntheticAsset()
