...
func (HandlerDonateSuite) TestDonateLiquidityAuction(c *C) {
	...
	// At the beginning we should have PendingRune, RuneDepositValue, AssetDepositValue and Units equal to zero.
	for _, lp := range lps {
	...
	lps = getLPs(c, testKeeper, w.ctx)
	// After we donate we should have something on RuneDepositValue, AssetDepositValue and Units.
	// PendingRune and PendingAsset should now be zero.
...
func (HandlerDonateSuite) TestDonateLiquidityAuctionBonded(c *C) {
	...
	// After we donate we should have the same values as before due the LP's aleady been bonded.
	for _, lp := range lps {
		c.Assert(lp.PendingCacao.IsZero(), Equals, true)
		c.Assert(lp.PendingAsset.Equal(cosmos.NewUint(10*common.One)), Equals, true)
		c.Assert(lp.CacaoDepositValue.IsZero(), Equals, true)
