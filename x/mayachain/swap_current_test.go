...
import (
	...
	thorchaintypes "gitlab.com/mayachain/mayanode/x/mayachain/types"
)
...
func (s *SwapV95Suite) TestSwap(c *C) {
	...
	inputs := []struct {
		...
		},
		{
			...
			target:        common.BaseAsset(),
			amount:        cosmos.NewUint(100 * common.One),
			requester:     GetRandomBNBAddress(),
			destination:   GetRandomBaseAddress(),
		...
		},
		{
			...
			amount:        cosmos.NewUint(100 * common.One),
			requester:     GetRandomBaseAddress(),
...
func QuoUint(num cosmos.Uint, denom cosmos.Uint) cosmos.Dec {
...
func (s *SwapV95Suite) TestSynthSwap_RuneSynthRune(c *C) {
	...
	pool.BalanceCacao = cosmos.NewUint(1111 * common.One)
	...
	// swap rune --> synth
	{
		...
		tx := common.NewTx(
			...
			addr,
			common.NewCoins(
				common.NewCoin(common.BaseAsset(), cosmos.NewUint(50*common.One)),
			),
			BNBGasFeeSingleton,
			"",
		)
		...
		luviBefore2 := pool.BalanceCacao.Mul(pool.BalanceAsset).Quo(poolUnitsBefore2)
		...
		initialBalanceCacao := cosmos.NewUint(1111 * 1e8)
		...
		nativeRuneFee := cosmos.NewUint(20 * 1e8)
		...
		assetFee := cosmos.NewUint(
			uint64(QuoUint(nativeRuneFee.Mul(newBalanceAsset),
				initialBalanceCacao.Add(swapAmt)).RoundInt64()))
		...
		numerator := swapAmt.Mul(TWO).Mul(initialBalanceAsset).Mul(TWO).Mul(initialBalanceCacao)
		denom := swapAmt.Add(TWO.Mul(initialBalanceCacao))
		...
		runeDisbursement := cosmos.NewUint(
			uint64(QuoUint(assetFee.Mul(initialBalanceCacao.Add(swapAmt)),
				newBalanceAsset.Add(assetFee)).RoundInt64()))
		expectedRuneBalance := initialBalanceCacao.Add(swapAmt).Sub(runeDisbursement)
		...
		c.Check(pool.BalanceCacao.Uint64(), Equals, expectedRuneBalance.Uint64(),
			Commentf("Actual: %d Exp: %d", pool.BalanceCacao.Uint64(), expectedRuneBalance.Uint64()))
		c.Check(pool.BalanceCacao.Uint64(), Equals, uint64(114133869605), Commentf("%d", pool.BalanceCacao.Uint64()))
		...
		luviAfter2 := pool.BalanceCacao.Mul(pool.BalanceAsset).Quo(poolUnitsAfter2)
	...
	// swap synth --> rune
	{
		...
		initialBalanceCacao := pool.BalanceCacao
		initialBalanceAsset := pool.BalanceAsset
		// For synths, the pool depths are double to decrease the fee.
		// swapResult: (swapAmt * 2*BalanceRune * 2*BalanceAsset ) / (swapAmt + 2*BalanceAsset )^2
		TWO := cosmos.NewUint(2)
		numerator := swapAmt.Mul(TWO).Mul(initialBalanceCacao).Mul(TWO).Mul(initialBalanceAsset)
		denom := swapAmt.Add(TWO.Mul(initialBalanceAsset))
		denom = denom.Mul(denom)
		swapResult := cosmos.NewUint(uint64(QuoUint(numerator, denom).TruncateInt64()))
		expBalanceCacao := initialBalanceCacao.Sub(swapResult)
		...
		amount, _, err := newSwapperV95().Swap(ctx, mgr.Keeper(), tx, common.BaseAsset(), addr, cosmos.ZeroUint(), "", "", nil, cosmos.NewUint(1000_000), 20_000, mgr)
		c.Assert(err, IsNil)
		c.Check(amount.Uint64(), Equals, swapResult.Uint64(),
			Commentf("Actual: %d Exp: %d", amount.Uint64(), swapResult.Uint64()))
		pool, err = mgr.Keeper().GetPool(ctx, common.BNBAsset)
		...
		c.Check(pool.BalanceCacao.Uint64(), Equals, expBalanceCacao.Uint64(),
			Commentf("Actual: %d Exp: %d", pool.BalanceCacao.Uint64(), expBalanceCacao.Uint64()))
		c.Check(pool.LPUnits.Uint64(), Equals, uint64(111100000000), Commentf("%d", pool.LPUnits.Uint64()))
		// We don't check pool.SynthUnits to not duplicate the calculation here,
		// but we did check BalanceAsset, LPUnits, and totalSynthSupply, the
		// three inputs to the calculation.
...
func (s *SwapV95Suite) TestSynthSwap_AssetSynth(c *C) {
	...
	pool := NewPool()
	pool.Asset = common.BNBAsset
	pool.BalanceCacao = cosmos.NewUint(111111 * common.One)
	pool.BalanceAsset = cosmos.NewUint(3400 * common.One)
	pool.LPUnits = pool.BalanceCacao
	c.Assert(mgr.Keeper().SetPool(ctx, pool), IsNil)
	...
	tx := common.NewTx(
		...
		addr,
		common.NewCoins(
			common.NewCoin(common.BNBAsset, cosmos.NewUint(5000*common.One)),
		),
		BNBGasFeeSingleton,
		"",
	)
	...
	TWO := cosmos.NewUint(2)
	numerator = swapResult1.Mul(TWO).Mul(balanceCacao1).Mul(TWO).Mul(balanceAsset1)
	denom = swapResult1.Add(TWO.Mul(balanceCacao1))
	denom = denom.Mul(denom)
	swapResult2 := cosmos.NewUint(uint64(QuoUint(numerator, denom).TruncateInt64()))
	balanceCacao2 := balanceCacao1.Add(swapResult1)
	balanceAsset2 := balanceAsset1
	assetFee := cosmos.NewUint(
		uint64(QuoUint(nativeRuneFee.Mul(balanceAsset2),
			balanceCacao2).RoundInt64()))
	runeDisbursement := cosmos.NewUint(
		uint64(QuoUint(assetFee.Mul(balanceCacao2),
			balanceAsset2.Add(assetFee)).RoundInt64()))
	expBalanceCacao := balanceCacao2.Sub(runeDisbursement) // BalanceRune after the second swap (rune->synth)
	...
	c.Check(amount.Uint64(), Equals, uint64(198584447660), Commentf("%d", amount.Uint64()))
	pool, err = mgr.Keeper().GetPool(ctx, common.BNBAsset)
	c.Assert(err, IsNil)
	...
	btcPool.BalanceCacao = cosmos.NewUint(common.One * 10)
	...
	c.Check(pool.BalanceCacao.Uint64(), Equals, uint64(10*common.One), Commentf("%d", pool.BalanceCacao.Uint64()))
	pool.CalcUnits(mgr.GetVersion(), totalSynthSupply)
