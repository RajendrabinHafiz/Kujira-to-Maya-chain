...
func Init() {
	...
	assert(viper.BindEnv("bifrost.mayachain.signer_name", "SIGNER_NAME"))
	...
	assert(viper.BindEnv("bifrost.mayachain.chain_id", "CHAIN_ID"))
	assert(viper.BindEnv("bifrost.mayachain.chain_host", "CHAIN_API"))
	assert(viper.BindEnv(
		"bifrost.mayachain.chain_rpc",
		"CHAIN_RPC",
	))
	assert(viper.BindEnv(
	...
	assert(viper.BindEnv(
		"bifrost.chains.DASH.rpc_host",
		"DASH_HOST",
	))
	assert(viper.BindEnv(
		"bifrost.chains.DASH.block_scanner.rpc_host",
		"DASH_HOST",
	))
	assert(viper.BindEnv(
		"bifrost.chains.DASH.block_scanner.start_block_height",
		"DASH_START_BLOCK_HEIGHT",
	))
	assert(viper.BindEnv(
	...
	assert(viper.BindEnv(
		"bifrost.chains.THOR.cosmos_grpc_host",
		"THOR_GRPC_HOST",
	))
	assert(viper.BindEnv(
		"bifrost.chains.THOR.block_scanner.cosmos_grpc_host",
		"THOR_GRPC_HOST",
	))
	assert(viper.BindEnv(
		"bifrost.chains.THOR.cosmos_grpc_tls",
		"THOR_GRPC_TLS",
	))
	assert(viper.BindEnv(
		"bifrost.chains.THOR.block_scanner.cosmos_grpc_tls",
		"THOR_GRPC_TLS",
	))
	assert(viper.BindEnv(
		"bifrost.chains.THOR.block_scanner.start_block_height",
		"THOR_START_BLOCK_HEIGHT",
	))
	assert(viper.BindEnv("bifrost.chains.GAIA.disabled", "GAIA_DISABLED"))
	assert(viper.BindEnv("bifrost.chains.BNB.disabled", "BNB_DISABLED"))
	assert(viper.BindEnv("bifrost.chains.BCH.disabled", "BCH_DISABLED"))
	assert(viper.BindEnv("bifrost.chains.THOR.disabled", "THOR_DISABLED"))
	assert(viper.BindEnv("bifrost.chains.DOGE.disabled", "DOGE_DISABLED"))
	assert(viper.BindEnv("bifrost.chains.DASH.disabled", "DASH_DISABLED"))
...
func InitBifrost() {
	...
	config.Bifrost.MayaChain.SignerPasswd = os.Getenv("SIGNER_PASSWD")
...
func InitThornode(ctx context.Context) {
	...
	// set paths
	home := os.ExpandEnv("$HOME/.mayanode")
...
func thornodeAutoStateSync(ctx context.Context) {
	...
	dataDir := os.ExpandEnv("$HOME/.mayanode/data/state.db")
